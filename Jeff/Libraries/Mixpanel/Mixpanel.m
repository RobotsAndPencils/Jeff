...
static void MixpanelReachabilityCallback(SCNetworkReachabilityRef target, SCNetworkReachabilityFlags flags, void *info)
{
    if (info != NULL && [(__bridge NSObject*)info isKindOfClass:[Mixpanel class]]) {
        ...
        RBKLog(@"Mixpanel reachability callback received unexpected info object");
    }
}
...
+ (Mixpanel *)sharedInstance
{
    ...
    if (sharedInstance == nil) {
        RBKLog(@"%@ warning sharedInstance called before sharedInstanceWithToken:", self);
    }
#endif
    return sharedInstance;
}
...
- (instancetype)initWithToken:(NSString *)apiToken andFlushInterval:(NSUInteger)flushInterval
{
    ...
    if ([apiToken length] == 0) {
        RBKLog(@"%@ warning empty api token", self);
    }
    if (self = [self init]) {
        ...
        if (!reachabilityOk) {
            RBKLog(@"%@ failed to set up reachability callback: %s", self, SCErrorString(SCError()));
        }
...
- (void)dealloc
{
    ...
    if (_reachability != NULL) {
        if (!SCNetworkReachabilitySetCallback(_reachability, NULL, NULL)) {
            RBKLog(@"%@ error unsetting reachability callback", self);
        }
        if (!SCNetworkReachabilitySetDispatchQueue(_reachability, NULL)) {
            RBKLog(@"%@ error unsetting reachability dispatch queue", self);
        }
...
- (NSData *)JSONSerializeObject:(id)obj
{
    ...
    @catch (NSException *exception) {
        RBKLog(@"%@ exception encoding api data: %@", self, exception);
    }
    if (error) {
        RBKLog(@"%@ error encoding api data: %@", self, error);
    }
    return data;
}
...
- (id)JSONSerializableObjectForObject:(id)obj
{
    ...
    if ([obj isKindOfClass:[NSDictionary class]]) {
        ...
        for (id key in obj) {
            ...
            if (![key isKindOfClass:[NSString class]]) {
                ...
                RBKLog(@"%@ warning: property keys should be strings. got: %@. coercing to: %@", self, [key class], stringKey);
            } else {
    ...
    RBKLog(@"%@ warning: property values should be valid json types. got: %@. coercing to: %@", self, [obj class], s);
    return s;
}
...
- (void)identify:(NSString *)distinctId
{
    if (distinctId == nil || distinctId.length == 0) {
        RBKLog(@"%@ error blank distinct id: %@", self, distinctId);
        return;
    }
...
- (void)createAlias:(NSString *)alias forDistinctID:(NSString *)distinctID
{
    if (!alias || [alias length] == 0) {
        RBKLog(@"%@ create alias called with empty alias: %@", self, alias);
        return;
    }
    if (!distinctID || [distinctID length] == 0) {
        RBKLog(@"%@ create alias called with empty distinct id: %@", self, distinctID);
        return;
    }
...
- (void)track:(NSString *)event properties:(NSDictionary *)properties
{
    if (event == nil || [event length] == 0) {
        RBKLog(@"%@ mixpanel track called with empty event parameter. using 'mp_event'", self);
        event = @"mp_event";
    }
...
- (void)flushQueue:(NSMutableArray *)queue endpoint:(NSString *)endpoint
{
    while ([queue count] > 0) {
        ...
        if (error) {
            RBKLog(@"%@ network failure: %@", self, error);
            break;
        }

        NSString *response = [[NSString alloc] initWithData:responseData encoding:NSUTF8StringEncoding];
        if ([response intValue] == 0) {
            RBKLog(@"%@ %@ api rejected some items", self, endpoint);
        };
...
- (void)archiveEvents
{
    ...
    if (![NSKeyedArchiver archiveRootObject:eventsQueueCopy toFile:filePath]) {
        RBKLog(@"%@ unable to archive events data", self);
    }
}
...
- (void)archivePeople
{
    ...
    if (![NSKeyedArchiver archiveRootObject:peopleQueueCopy toFile:filePath]) {
        RBKLog(@"%@ unable to archive people data", self);
    }
}
...
- (void)archiveProperties
{
    ...
    if (![NSKeyedArchiver archiveRootObject:p toFile:filePath]) {
        RBKLog(@"%@ unable to archive properties data", self);
    }
}
...
- (void)unarchiveEvents
{
    ...
    @catch (NSException *exception) {
        RBKLog(@"%@ unable to unarchive events data, starting fresh", self);
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:filePath]) {
        ...
        if (!removed) {
            RBKLog(@"%@ unable to remove archived events file at %@ - %@", self, filePath, error);
        }
    }
    if (!self.eventsQueue) {
        self.eventsQueue = [NSMutableArray array];
...
- (void)unarchivePeople
{
    ...
    @catch (NSException *exception) {
        RBKLog(@"%@ unable to unarchive people data, starting fresh", self);
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:filePath]) {
        ...
        BOOL removed = [[NSFileManager defaultManager] removeItemAtPath:filePath error:&error];
        if (!removed) {
            RBKLog(@"%@ unable to remove archived people file at %@ - %@", self, filePath, error);
        }
    }
    if (!self.peopleQueue) {
        self.peopleQueue = [NSMutableArray array];
...
- (void)unarchiveProperties
{
    ...
    @catch (NSException *exception) {
        RBKLog(@"%@ unable to unarchive properties data, starting fresh", self);
    }
    if ([[NSFileManager defaultManager] fileExistsAtPath:filePath]) {
        NSError *error;
        BOOL removed = [[NSFileManager defaultManager] removeItemAtPath:filePath error:&error];
        if (!removed) {
            RBKLog(@"%@ unable to remove archived properties file at %@ - %@", self, filePath, error);
        }
    }
    if (properties) {
